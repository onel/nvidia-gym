...
:::{note}
The configuration file (`CONFIG_PATH`) and training script (`run_grpo_nemo_gym.py`) referenced below are located in the NeMo RL repository that you cloned during the {doc}`Setup <setup>` step, not in the Gym repository.
:::
